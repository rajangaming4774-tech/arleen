...
## Design
After a reference layout the owner sent: off-white ground (`--paper`), rounded photo cards (`--radius`), a tall condensed all-caps display face (`Bebas Neue`) with `Manrope` for text and labels, and one orange accent (`--accent`) for emphasis words (`<em>` inside a heading), kickers' `//` mark, hovers and the call button; only the footer is dark. Home: the hero card carries the huge title top-left, the tagline and a pill button bottom-left, a projects badge and a years stat card; then an "about" block (label, right-aligned uppercase statement with an orange phrase, figures / text / button), three project cards, one wide card and a closing statement (`.about`, `.projectsBlock` in `build.mjs`). Section labels sit in a fixed left margin column (`--margin`, see `.section-head`). Inner pages open with a `.cover` (headline on paper, photo offset right with a caption) and a stat rail; the divisions are three cards (`.rows--columns`); the projects page uses an editorial grid (`.work--editorial`: first tile wide, second tall, rest three-up); buttons are pills (`.btn`, `.btn--light`). The header is a fixed paper bar on every page; the grid button opens a full-screen menu. Motion is one soft fade-up per section, switched off for "reduce motion".
